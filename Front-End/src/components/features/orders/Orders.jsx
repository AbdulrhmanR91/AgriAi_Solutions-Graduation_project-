    ...
    return (
        <div className="min-h-screen bg-gray-50 p-4 sm:p-6 pb-24 ">
            <div className="max-w-7xl mx-auto mb-16"> {/* Added mb-16 */}
                <h1 className="text-xl sm:text-2xl font-bold text-gray-800 mb-4 sm:mb-6">{t('farmer.orders.title')}</h1>
                ...
                <div className="flex gap-2 sm:gap-4 mb-4 sm:mb-6 ">
                    ...
                    >
                        {t('farmer.orders.orders_received')}
                    </button>
                    <button
                ...
                {activeTab === 'received' ? (
                    orders.length === 0 ? (
                        <p className="text-gray-500">{t('farmer.orders.no_orders_received')}</p>
                    ) : (
                                ...
                                <div key={order._id} className="bg-white rounded-lg shadow-sm p-3 sm:p-4">
                                    <div className="flex flex-col sm:flex-row items-start gap-3 sm:gap-4">
                                        ...
                                        <div className="flex items-center gap-2 w-full sm:w-auto">
                                            <img
                                                src={order.buyer?.profileImage ? getImageUrl(order.buyer.profileImage) : user}
                                                alt={order.buyer?.name || t('farmer.orders.buyer')}
                                            ...
                                            <div>
                                                ...
                                                <p className="text-xs sm:text-sm text-gray-500">{t('farmer.orders.buyer')}</p>
                                            </div>
                                        </div>
                                        ...
                                        <div className="flex-1 w-full">
                                            <div className="flex items-center gap-3">
                                                <img
                                                    src={order.product?.image ? getImageUrl(order.product.image) : user}
                                            ...
                                            {order.shippingDetails && (
                                                <div className="mt-3 text-xs sm:text-sm text-gray-600 grid grid-cols-1 sm:grid-cols-2 gap-1 sm:gap-2">
                                                    <p>{t('farmer.orders.address')}: {order.shippingDetails.address || 'N/A'}</p>
                                                    <p>{t('farmer.orders.city')}: {order.shippingDetails.city || 'N/A'}</p>
                                                    <p>{t('farmer.orders.phone')}: {order.shippingDetails.phone || 'N/A'}</p>
                                                    {order.shippingDetails.notes && (
                                                        <p className="col-span-full">{t('farmer.orders.notes')}: {order.shippingDetails.notes}</p>
                                                    )}
                                                </div>
                                            )}
                                        </div>
                    ...
                    myOrders.length === 0 ? (
                        <p className="text-gray-500">{t('farmer.orders.no_orders_placed')}</p>
                    ) : (
                        <div className="grid gap-4">
                            {myOrders.map(order => (
                                <div key={order._id} className="bg-white rounded-lg shadow-sm p-3 sm:p-4">
                                    <div className="flex flex-col sm:flex-row items-start gap-3 sm:gap-4">
                                        ...
                                        <div className="flex items-center gap-2 w-full sm:w-auto">
                                            <img
                                                src={order.seller?.profileImage ? getImageUrl(order.seller.profileImage) : user}
